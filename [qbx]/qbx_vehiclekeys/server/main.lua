...
lib.callback.register('qbx_vehiclekeys:server:carjack', function(source, netId, weaponTypeGroup)
    local vehicle = NetworkGetEntityFromNetworkId(netId)
    if not IsPlayerNearVehicle(source, vehicle) then return end
    -- Block: agar attacker khud gaadi ke andar hai to carjack nahi milega
    local attackerPed = GetPlayerPed(source)
    if attackerPed ~= 0 and GetVehiclePedIsIn(attackerPed, false) ~= 0 then
        return false
    end
