...
local function carjackVehicle(driver, vehicle)
    -- Block: agar attacker khud gaadi ke andar baitha hai to carjack nahi hoga
    if cache.vehicle or IsPedInAnyVehicle(cache.ped, false) then return end
...
local function watchCarjackingAttempts()
    ...
    CreateThread(function()
        while cache.weapon and not getIsBlacklistedWeapon(cache.weapon) do
            -- Agar player khud gaadi me baitha hai to carjack check skip karo
            if cache.vehicle or IsPedInAnyVehicle(cache.ped, false) then
                Wait(500)
            else
